...
def test_relation_valid_with_observed_cardinality_and_apta() -> None:
    ...
    frames = {
        "wo_afes": _pd().DataFrame({"wonum": ["A", "B", "C"]}),
        "admafecost": _pd().DataFrame({"wonum": ["A", "A", "B", "C"]}),
    }
...
def test_relation_detects_nulls_and_percentual() -> None:
    ...
    schema = _schema_with_single_relation()
    frames = {
        "wo_afes": _pd().DataFrame({"wonum": ["A", None, "B", None]}),
        "admafecost": _pd().DataFrame({"wonum": ["A", "B", "C", None]}),
    }
...
def test_relation_detects_unmatched_samples() -> None:
    ...
    schema = _schema_with_single_relation()
    frames = {
        "wo_afes": _pd().DataFrame({"wonum": ["A", "X", "Y"]}),
        "admafecost": _pd().DataFrame({"wonum": ["A", "B"]}),
    }
...
def test_relation_detects_duplicates_per_side() -> None:
    ...
    schema = _schema_with_single_relation()
    frames = {
        "wo_afes": _pd().DataFrame({"wonum": ["A", "A", "B"]}),
        "admafecost": _pd().DataFrame({"wonum": ["A", "B", "B", "B"]}),
    }
...
def test_relation_recommendation_nao_apta_for_low_match() -> None:
    ...
    schema = _schema_with_single_relation()
    frames = {
        "wo_afes": _pd().DataFrame({"wonum": ["A", "B", "C", "D"]}),
        "admafecost": _pd().DataFrame({"wonum": ["X", "Y", "Z", "A"]}),
    }
