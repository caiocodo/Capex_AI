...
def test_execute_join_left_has_trace_and_unmatched_warning() -> None:
    ...
    left_df = _pd().DataFrame({"k": ["A", "B", "C"], "v1": [1, 2, 3]})
    right_df = _pd().DataFrame({"k2": ["A", "B"], "v2": [10, 20]})
...
def test_execute_join_inner_reduces_rows_and_warns() -> None:
    ...
    left_df = _pd().DataFrame({"k": ["A", "B", "C"]})
    right_df = _pd().DataFrame({"k2": ["A", "B"]})
...
def test_execute_join_anti_returns_only_unmatched_left() -> None:
    ...
    left_df = _pd().DataFrame({"k": ["A", "B", "C"], "v": [1, 2, 3]})
    right_df = _pd().DataFrame({"k2": ["A", "B"]})
...
def test_materialize_base_views_returns_expected_views_and_trace() -> None:
    ...
    frames = {
        ...
        "inv_afe": _pd().DataFrame({"refwo": ["WO1"], "invoicecostid": [1]}),
        ...
        "invoicecost": _pd().DataFrame({"CHAVE_WO_CODE": ["C1", "C9"], "linecost": [100, 200]}),
    }
